      ...
      if(G_eAntApiCurrentMessageClass == ANT_DATA)
      {
        ...
        { 
          ...
          case 0:
            ...
            LCDMessage(LINE2_START_ADDR, "Heart rate is:"); 
            LCDMessage(LINE2_START_ADDR+14, au8DataContent_Rate);
            
            LCDMessage(LINE1_START_ADDR,"Battery:");
            LCDMessage(LINE1_START_ADDR+9, au8DataContent_Battery);
            ...
            if(G_au8AntApiCurrentMessageBytes[7] >= 180||G_au8AntApiCurrentMessageBytes[7] <= 30)
            {
              u32ledFrequency=G_au8AntApiCurrentMessageBytes[7];
             PWMAudioSetFrequency(BUZZER1, 262);
              PWMAudioOn(BUZZER1);
            }
            else if(G_au8AntApiCurrentMessageBytes[7] >= 120||G_au8AntApiCurrentMessageBytes[7] <= 60)
            { 
             u32ledFrequency=G_au8AntApiCurrentMessageBytes[7];
              LedOn(YELLOW);
          ...
          case 7:
            ...
            LCDMessage(LINE1_START_ADDR + 12, "%");
            LCDMessage(LINE1_START_ADDR+9, au8DataContent_Battery);
            if(G_au8AntApiCurrentMessageBytes[1] <= 20)
            {
             LedOn(RED);
             LedOff(GREEN);
            }
            else if(G_au8AntApiCurrentMessageBytes[1] >= 95)
            {
              LedOn(GREEN);
              LedOff(RED);
            }
